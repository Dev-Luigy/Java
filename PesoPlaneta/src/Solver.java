...
enum Planeta {
    ...
    private Planeta(double massa, double radius) {
        this.massa = massa;
        this.radius = radius;
    }
...
class Pessoa {
    ...
    public double massaEquivalente(Planeta otherPlanet) {
        System.out
                .format("A força gravitacional no planeta %s é %.2f\n", otherPlanet, otherPlanet.forcaGravitacional());
        return (mass / p.forcaGravitacional()) * otherPlanet.forcaGravitacional();
    }

    public String toString() {
        return String.format("nome: %s mass: %.2f planeta: %s", nome, mass, p);
    }
}
